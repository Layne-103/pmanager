...
│   │   │   └── ticket_tag.py        # Association table
│   │   │
│   │   ├── schemas/
│   │   │   ├── __init__.py
...
│           ├── 002_create_tags_table.py
│           ├── 003_create_ticket_tags_table.py
│           └── 004_create_triggers.py
│
├── client/
│   ├── index.html
│   ├── package.json
...
def get_tickets(
    ...
    return ticket_service.get_tickets(db, search, tag_ids, status)
...
def add_tags_to_ticket(ticket_id: int, tag_ids: List[int], db: Session = Depends(get_db)):
    """Add tags to a ticket"""
    return ticket_service.add_tags(db, ticket_id, tag_ids)
...
#### 8.5.6 Configuration (`app/config.py`)
```python
